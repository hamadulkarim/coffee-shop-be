describe 'PATCH /api/v1/line_items/:id/reduce', { type: :request, skip_request: true } do
  ...
  let!(:line_item) { create(:line_item, cart: cart, quantity: 3) }

  context 'with user not signed in' do
    it 'does not render decrement template' do
      request
  ...
  context 'with user signed in' do
    ...
    it 'returns a successful response with decrement template' do
      request

      expect(response.status).to eq(200)
      expect(response).to have_http_status(:ok)
      expect(response).to render_template('decrement')
